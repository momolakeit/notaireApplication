...
class ITextServiceTest {
    ...
    public void testSignature() throws GeneralSecurityException, IOException {
        assertNotNull(iTextService.sign(TestDocumentUtils.initPDFDocument(),"me donner le cash en cash","Montreal"));
        File f = new File(ITextService.DEST+ITextService.RESULT_FILES);
        assertTrue(f.exists());
